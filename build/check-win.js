...
function checkHorizontalWin(gameGrid, mark) {
    for (let row = 0; row < gameGrid.length; ++row) {
        if (gameGrid[row][0] === mark
            && gameGrid[row][1] === mark
            && gameGrid[row][2] === mark)
            return true;
    }
...
;
function checkVerticalWin(gameGrid, mark) {
    for (let col = 0; col < gameGrid.length; ++col) {
        if (gameGrid[0][col] === mark
            && gameGrid[1][col] === mark
            && gameGrid[2][col] === mark)
            return true;
    }
...
;
function checkDiagonalWin(gameGrid, mark) {
